...
class CotizacionController extends Controller
{
    ...
    /**
     * Generar PDF de la proforma (Cotización con datos alterados)
     */
    /**
     * Obtener detalles de la cotización para el modal de precios manuales
     */
    public function getDetails(Cotizacion $cotizacione)
    {
        Log::info('getDetails called for Cotizacion ID: ' . $cotizacione->id);

        $cotizacione->load('productos');
        
        $productos = $cotizacione->productos->map(function ($producto) {
            return [
                'id' => $producto->id,
                'codigo' => $producto->codigo,
                'nombre' => $producto->nombre,
                'cantidad' => $producto->pivot->cantidad,
                'precio_actual' => $producto->pivot->precio_unitario,
            ];
        });

        // Configuración actual
        $config = ConfiguracionProforma::where('sucursal_id', $cotizacione->sucursal_id)->first();
        $metodo = $config ? $config->metodo_precio : 'ninguno';

        Log::info('Config found: ', ['config' => $config, 'metodo' => $metodo]);
     ...
     * Generar PDF de la proforma (Cotización con datos alterados)
     */
    public function generarProforma(Request $request, Cotizacion $cotizacione)
    {
        Log::info('generarProforma called', ['id' => $cotizacione->id, 'inputs' => $request->all()]);
        ...
        // Buscar configuración de proforma para esta sucursal
        $configProforma = ConfiguracionProforma::where('sucursal_id', $cotizacione->sucursal_id)->first();
        
        Log::info('Config Loaded', ['config' => $configProforma]);

        // Lógica de Precios
        if ($configProforma) {
            // Caso 1: Manual (precios vienen en el Request)
            if ($configProforma->metodo_precio === 'manual' && $request->has('custom_prices')) {
                Log::info('Applying Manual Prices');
                $customPrices = $request->input('custom_prices'); // array [producto_id => precio]
                
                $nuevoSubtotal = 0;
                
                foreach ($cotizacione->productos as $producto) {
                    if (isset($customPrices[$producto->id])) {
                        // Inyectar precio temporalmente
                        $nuevoPrecio = (float) $customPrices[$producto->id];
                        $producto->pivot->precio_proforma = $nuevoPrecio;
                        $producto->pivot->subtotal_proforma = $nuevoPrecio * $producto->pivot->cantidad;
                        
                        $nuevoSubtotal += $producto->pivot->subtotal_proforma;
                    } else {
                        // Fallback al precio original
                         $producto->pivot->precio_proforma = $producto->pivot->precio_unitario;
                         $producto->pivot->subtotal_proforma = $producto->pivot->subtotal;
                         $nuevoSubtotal += $producto->pivot->subtotal_proforma;
                    }
                }
                
                // Recalcular totales temporales para la vista
                $cotizacione->subtotal_proforma = $nuevoSubtotal;
                $cotizacione->total_proforma = $nuevoSubtotal; // Ignorando impuestos/descuentos complejos por ahora para proforma simple
                Log::info('New Total (Manual): ' . $nuevoSubtotal);
            
            // Caso 2: Porcentaje
            } elseif ($configProforma->metodo_precio === 'porcentaje') {
                 Log::info('Applying Percentage Pricing');
                 
                 // Usar valores del Request si existen (override del modal), si no, usar config
                 $valorPorcentaje = $request->input('override_valor', $configProforma->valor_porcentaje);
                 $tipoPorcentaje = $request->input('override_tipo', $configProforma->tipo_porcentaje);

                 if ($valorPorcentaje > 0) {
                     $factor = ($valorPorcentaje / 100);
                     if ($tipoPorcentaje === 'descuento') {
                         $factor = -$factor;
                     }
                     ...
                     foreach ($cotizacione->productos as $producto) {
                         ...
                         $nuevoPrecio = $precioOriginal + ($precioOriginal * $factor);
                         
                         // Guardar en propiedad temporal
                         $producto->pivot->precio_proforma = $nuevoPrecio;
                         $producto->pivot->subtotal_proforma = $nuevoPrecio * $producto->pivot->cantidad;
                     ...
                     $cotizacione->subtotal_proforma = $nuevoSubtotal;
                     $cotizacione->total_proforma = $nuevoSubtotal; 
                     Log::info('New Total (Percentage): ' . $nuevoSubtotal);
                 }
            }
        }
